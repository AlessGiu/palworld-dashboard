...
REMOTE_OLD_DIR = REMOTE_BASE + "/D5C16DC3464E7CC492225ABB991F1FA2_BEFORE_HOSTFIX"

LOCAL_DIR = os.path.join(os.path.dirname(__file__), "remote_save")

transport = paramiko.Transport((HOST, PORT))
transport.connect(username=USER, password=PASS)
sftp = paramiko.SFTPClient.from_transport(transport)

# 1. rename remote original folder aside (safety net)
try:
